...
class MakemkvconSpawnBase(_SpawnMixin):
    ...
    def get_messages_pattern_dict(self):
        pattern_dict = collections.OrderedDict([
            ...
            (fr'^Downloading latest HK to (?P<dest_dir>[^\r\n]+) \.\.\.{re_eol}', True),
            ...
            (fr'^The source file \'(?P<source_file>.*)\' is corrupt or invalid at offset (?P<offset>\d+)(?:-(?P<offset2>\d+))?, attempting to work around{re_eol}', self.generic_warning),
            ...
            (fr'^AV sync issue in stream (?P<stream_no>{re_stream_no}) at (?P<timestamp>\S+) *: too many video frames with invalid timecodes, future messages will be suppressed{re_eol}', self.generic_warning),
            (fr'^AV sync issue in stream (?P<stream_no>{re_stream_no}) at (?P<timestamp>\S+) *: Forced subtitles track #(?P<stream_no2>{re_stream_no}) turned out to be empty and was removed from output file{re_eol}', self.generic_warning),
            (fr'^AV sync issue in stream (?P<stream_no>{re_stream_no}) at (?P<timestamp>\S+) *: Track #(?P<stream_no2>{re_stream_no}) turned out to be empty and was removed from output file{re_eol}', self.generic_warning),
