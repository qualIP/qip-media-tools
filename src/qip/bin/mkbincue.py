...
def main():
    ...
    pgroup.add_argument('--device-sample-offset', default=6, help='sample read offset for your device (http://www.accuraterip.com/driveoffsets.htm)')
    pgroup.add_bool_argument('--cddb', help='enable CDDB (closed!)')
    ...
    pgroup.add_argument('--force-read-speed', default=None, help='force CDROM read speed')
    pgroup.add_argument('--safecopy-timing', default=False, action='store_true', help='write safecopy timing files')
    pgroup.add_argument('--cdparanoia-max-skip-retries', default=None, type=int, help='number of retries before cdparanoia is allowed to skip a sector')
    pgroup.add_argument('--no-disable-paranoia', default=False, action='store_true', help='do not use cdparanoia --disable-paranoia on first try')
    pgroup.add_argument('--max-track-retries', default=None, type=int, help='number of retries before giving up reading a track')
    pgroup.add_argument('--rebuild', default=False, action='store_true', help='rebuild tracks from best sectors if all else fails')
    pgroup.add_argument('--rebuild-unique-sectors', default=False, action='store_true', help='rebuild tracks even from unique sectors')
